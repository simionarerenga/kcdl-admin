  ...
  return (
    <div>
      ...
      {Object.entries(byIsland).map(([island, items]) => {
        ...
        const byCoop = groupBy(items, 'cooperative_name');
        return (
...
export default function ReportsCentre() {
  ...
  const reportEl = useMemo(() => {
    ...
    switch (selected) {
      ...
      case 'twc_summary':    return <div><div className="report-section-title">TWC Summary</div>{/* similar to CPR */}</div>;
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">Generate, preview and print all operational reports</div>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'280px 1fr', gap:20, alignItems:'start' }}>
        ...
        <div>
          <div className="card" style={{ padding:14 }}>
            <div className="card-title" style={{ marginBottom:12, padding:'0 4px' }}>Select Report</div>
            {REPORT_TYPES.map(r => (
              <button key={r.id} type="button"
                ...
                style={{
                  ...
                  background: selected===r.id ? 'var(--teal-dim)':'none',
                  color: selected===r.id ? 'var(--teal-light)':'var(--text-secondary)',
                  textAlign:'left', marginBottom:2, transition:'all 0.15s',
                }}
              >
                <span style={{ fontSize:'1rem', flexShrink:0 }}>{r.icon}</span>
                <div>
                  <div style={{ fontWeight:700, fontSize:'0.82rem', color: selected===r.id?'var(--teal-light)':'var(--text-primary)' }}>{r.label}</div>
                  <div style={{ fontSize:'0.72rem', color:'var(--text-muted)', marginTop:2, lineHeight:1.4 }}>{r.desc}</div>
                </div>
              </button>
            ))}
          </div>
        ...
        <div>
          {!selected && (
            <div className="card empty-state" style={{ padding:60 }}>
              ...
              <div className="empty-state-text">Select a report type from the list to preview and print it</div>
            </div>
          )}
